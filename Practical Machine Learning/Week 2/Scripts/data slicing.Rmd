...
Note that the partitioning does have to do anything wiht the K-folds algorithm below. In the example below it gets the imput spam$type and it partitioned it into 10 folds. Each training fold has 90% of the data, while the test fold has 10% of the data. The percentage would change if k is changed. 

```{r kfold,dependson="loadPackage"}
set.seed(32323)
folds <- createFolds(y=spam$type,k=10,list=TRUE,returnTrain=TRUE)
sapply(folds,length)
round(sapply(folds,length)/dim(spam)[1],1)
folds[[1]][1:10]
```
